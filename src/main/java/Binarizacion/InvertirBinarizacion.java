...
public class InvertirBinarizacion implements  ProcesarImagen{
    ...
    public BufferedImage[] convertir(BufferedImage imagen, int[] umbral)  {
        ...
        for (int yImg = 0; yImg < alto; yImg++) {
            for (int xImg = 0; xImg < ancho; xImg++) {
                ...
                int r = (rojo >=  umbral[0]) ? 0 : 255;
                int g = (verde >= umbral[0]) ? 0 : 255;
                int b = (azul >=  umbral[0]) ? 0 : 255;
                int gris = ((rojo+verde+azul)/3)>= umbral[0] ?  0 : 255;
